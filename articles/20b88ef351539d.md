...
`static` は、メソッドやフィールドをインスタンスに置かず、クラスオブジェクト側へ定義する構文です。「どの個体にも属さず、クラスという名前から呼ぶ責務か」を表せます。
...
`user.createGuest()` は呼べず、`User.greet()` も呼べません。staticかどうかはアクセス制御の強さを決めるものではありません。処理の受け手がクラスか個体かを決めています。
...
インスタンスを作らなくても、static initializerは実行されます。そのため、モジュールimport時にclassが評価されるだけで重い計算、通信、ログ送信などが起きる設計は避けます。importの副作用は起動順序やテスト分離を難しくします。
...
method内の `this` は、staticではインスタンスを指しません。method呼び出しのreceiverとなったクラスオブジェクトです。継承すると、この違いが表面化します。
...
定義元へ常に固定したい処理では、`this` を使わずclass名を明示します。
...
methodを変数へ切り離した場合も、receiverは失われます。
...
生成方法へ名前を付ける用途では、static methodがよく使われます。
...
複数文、分岐、例外処理を伴うstatic初期化は、static initialization blockでclass内へまとめられます。class評価時に上から実行され、一つのclassに複数書けます。
...
単純な式なら通常のstatic field initializerで十分です。書けるからといって、static blockを使う必要はありません。class内部のprivate状態を含む複数文の同期初期化を一か所に閉じ込めたいときに限定します。
...
また、classの `implements` は基本的にinstance側を検査し、static memberの契約までは検査しません。static factoryを持つclassの契約は、constructor signatureを含む別interfaceへ `typeof SomeClass` を代入して確認します。
...
generic classの型パラメーターはinstanceごとの型を表すため、static memberから直接使えません。static fieldは全instanceで一つです。型ごとのstatic cacheが欲しい設計では、runtimeで型情報が消えることも踏まえ、明示的なkeyや外部registryを検討します。
...
`static` の本質は「便利な関数置き場」ではありません。memberの所有者をinstanceからclass objectへ変えることです。
...
「クラス名から呼ぶと見栄えがよい」だけで判断しないようにします。値の寿命、共有範囲、継承時の契約を説明できるかで判断すると、staticの過剰利用を避けながら必要な場所で活用できます。
