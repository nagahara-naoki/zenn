---
title: "Angular Signals完全入門――signal・computed・effectの正しい使い分け"
emoji: "📶"
type: "tech"
...
少し前までは、ただのプロパティで書いていました。

```ts
quantity = 1;
```

違いは`signal()`で包んだかどうかだけに見えます。ただし、この2つが持っている機能は違います。
...
この違いが効くのが、Angular 21以降の新規プロジェクトで既定になったZonelessです。Zonelessでは、Angularが非同期処理を監視しません。「ここが変わった」という通知を受けて画面を更新します。通知を出せるのがSignalです。状態をSignalで持つのが前提になったのは、このためです。
...
これは制限ではありません。保証です。`total`の値を決めるのは、宣言に書いた計算式だけです。ほかのどこかで別の値を入れられる余地がありません。合計がおかしいときに読む場所は、1か所に決まります。
...
Signalを覚えたあとで、もっとも書きやすい間違いがこれです。合計を`signal()`で持ち、`effect()`で埋める形です。
...
このコードは動きます。数量を増やせば、合計も変わります。それでも`computed()`へ直すべき理由が3つあります。
...
迷ったときの問いはひとつです。この値は自分で決めるのか、ほかの値から決まるのか。決まるものは`computed()`へ置く。そう決めておけば、状態の置き場所はほとんど自動的に定まります。
