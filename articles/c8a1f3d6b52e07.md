...
最初に手を付けるのは、このうち`src/app/`の中だけです。残りは設定ファイルで、しばらく開く必要はありません。

とはいえ、開かないファイルが何をしているか分からないままだと、エラーが出たときに手が止まります。この記事では、生成されたファイルを役割ごとに分けて、それぞれが何を担っているかを見ていきます。あわせて、ブラウザでURLを開いてから画面が出るまでの流れも追います。前提はAngular 22です。
...
生成されたファイルを一度に理解する必要はありません。「これはどの層のファイルか」が分かれば、必要になったときに開けます。まずは`src/app/`から始めて、設定ファイルは触る用が出てきたときに読めば十分です。
