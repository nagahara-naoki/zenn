...
安心してください。最初に手を付けるのは`src/app/`の中だけです。残りは設定ファイルで、しばらく開く必要はありません。

とはいえ、開かないファイルが何をしているか分からないままだと、エラーが出た瞬間に手が止まります。「どこを直せばいいのかが分からない」という止まり方です。

この記事では、生成されたファイルを役割ごとに仕分けて、それぞれが何を担っているかを見ていきます。あわせて、ブラウザでURLを開いてから画面が出るまでの流れも追います。前提はAngular 22です。
