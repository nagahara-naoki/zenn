...
DIという考え方そのものは、Angularに限った話ではありません。フレームワークに依存しない説明は別記事『DIとは？依存関係の注入をわかりやすく理解する』で扱っています。この記事ではAngular 22を前提に、`@Service()`と`inject()`を書いたときAngularの内部で何が起きるかを追います。読み終えると、Serviceのインスタンスがどこで生まれ、どこまで共有され、見つからないときに何が起きるかを自分の言葉で説明できるようになります。
