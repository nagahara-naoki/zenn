...
誰が、いつ、そのインスタンスを作っているのでしょうか。作った後、それはどこに置かれているのでしょうか。

答えを持っているのはインジェクターです。`inject()`は「作って」という命令ではありません。「このトークンに対応するインスタンスをください」という要求です。要求を受けたインジェクターが、必要なら生成し、すでに持っていればそれを返します。`new`が消えたわけではなく、`new`を呼ぶ場所が呼び出し側からインジェクターへ移っただけです。
...
`inject(CartStore)`という1行は、インスタンスの生成をインジェクターへ委ねる宣言です。誰がいつ作るかを手放した見返りに、共有範囲と差し替えの自由が手に入ります。Serviceの挙動に迷ったら、そのトークンがどのインジェクターに登録されているかを確認してください。答えはほとんどそこにあります。
