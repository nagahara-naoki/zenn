...
このコードは動きます。問題は、その後です。

画面に値を表示するためだけに`subscribe()`し、受け取った値を別の`Signal`（Angularが変更を追跡する状態）やプロパティへ移すと、購読解除や、エラー・ローディング状態、再取得の管理までコンポーネントが引き受けることになります。
...
移せる購読はAngularへ任せ、副作用の境界だけを手元に残す。この分け方なら、`subscribe()`の数ではなく、非同期処理の責任範囲でコードを判断できます。
