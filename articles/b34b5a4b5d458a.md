...
問題は分岐そのものより、各分岐が独立した変更理由を持ち始めることです。プレミアム会員はキャンペーン期間だけ割引率が上がる、社員割引には上限がある、将来の会員種別は外部設定から率を読む、といった要件が入ると、1つの関数が日付、設定、上限、丸め規則まで抱えます。
...
各方式を同じ型の関数として実装します。外部の可変状態を暗黙に読まず、日時など結果に影響する値を引数で受け取ると、再現可能なテストを書けます。
...
実務では丸め規則、通貨、税込・税抜の順序も契約へ含めます。Strategyごとに勝手な単位を返せる設計は交換可能とは言えません。引数と戻り値の型に加えて、意味と失敗の表現も一致している必要があります。

## 依存や状態を持つStrategy

関数が設定やサービスを閉じ込める場合は、クロージャで生成できます。たとえば運用中に配布される割引率を、呼び出しごとに読む代わりにStrategy生成時に固定します。
...
ほかにも、値だけが違うなら設定表で十分です。Strategyが向くのは「データの差」より「手順の差」を表したいときです。継承で親classのメソッドを上書きするTemplate Methodも代替ですが、実行時に差し替えにくく、継承階層へ結合します。独立した関数を合成できるJavaScriptでは、まず関数注入を検討すると小さく保てます。
