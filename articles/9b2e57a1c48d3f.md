...
デコレーターが関数になっただけに見えます。実際は、受け渡しの仕組みそのものが変わりました。`input()`が返すのはSignalです。テンプレートで`price()`と読めば、Angularは「この表示は`price`に依存している」と記録します。`computed()`の中からも読めます。値の到着を`ngOnChanges`で待つ必要がなくなりました。
...
これは制限ではなく、境界の宣言です。`price`の値を決めるのは親だけ。子は表示に使うだけ。誰が値を持つのかが型で固定されます。金額がおかしいときに調べる場所は、親の1か所に決まります。
...
迷ったときの問いはひとつです。この値は誰のもので、どちらへ流れるのか。親のものなら`input()`。値ではなく出来事なら`output()`。両方が書くなら`model()`。そう決めておけば、コンポーネントの境界の設計はほとんど自動的に定まります。
