---
title: "JavaScriptのprototypeを探索規則から理解する"
emoji: "🔗"
type: "tech"
...
`object.method()`が動いても、そのメソッドが`object`自身に保存されているとは限りません。見つからなければ、JavaScriptは親にあたるオブジェクトを順にたどって探します。
...
`object.method()`が動いても、それが`object`の中にあるとは限りません。`prototype`は、複雑な継承だけに使う特殊機能ではなく、**普段のプロパティ読み取りを支える探索規則**です。
