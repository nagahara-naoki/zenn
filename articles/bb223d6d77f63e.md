...
`package.json` に書いたのは、5個。

`node_modules` を開いたら、1200個ありました。しかも430MB。ネットで見かける「ブラックホール」というジョークは、あながち冗談でもないなと思った瞬間です。
...
パッケージAが `utility@^1` を、パッケージBが `utility@^2` を要求している。この場合、どちらか一方に寄せることはできません。
...
あれはnpmが無駄なファイルを溜め込んでいた結果ではありませんでした。依存ツリー、複数バージョン、開発ツール、複数形式、バイナリ。5つの理由が重なった、当然の姿です。
