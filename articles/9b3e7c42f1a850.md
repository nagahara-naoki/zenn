---
title: "*ngIfはもう使わない――Angularの制御フロー@ifと@for"
emoji: "🔀"
type: "tech"
...
下のほうが読みやすい、で終わる話ではありません。`*ngIf`はAngular 20で非推奨になりました。API リファレンスには「`@if`ブロックを使ってください。将来のメジャーリリースで削除する予定です」と書かれています。

とはいえ、日本語の情報はまだ`*ngIf`で書かれたものが大半です。移行の判断に必要な情報を、この記事にまとめます。Angular 22を前提に、`@if`・`@for`・`@switch`の書き方と、`@for`で必須になった`track`の意味を扱います。

`track`は単なる記法の変更ではありません。指定を間違えると、画面が壊れます。この記事の半分はその話です。
...
DOMが作り直されると、コストだけの問題では済みません。フォーカス、テキストの選択範囲、`iframe`の中身、Componentが内部に持っている状態が、すべて失われます。一覧の中で入力欄に文字を打っている最中に配列が更新されると、その文字が消えます。
...
`track`に何を指定するかは、記法の選択ではなく、データの設計の話です。一覧に出すデータには一意なIDを持たせる。それだけで、この記事で挙げた不具合のほとんどは起きません。
