...
下のほうが読みやすい。それはそうなんですが、話はそこで終わりません。
...
冒頭で「指定を間違えると画面が壊れる」と書きました。その正体が、`track $index` の行ずれと `track item` の全再生成です。

そしてこの2つは、テンプレートの書き方の問題ではありませんでした。**一覧に出すデータに、一意なIDがあるかどうか**の問題です。
