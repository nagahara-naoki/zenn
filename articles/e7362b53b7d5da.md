...
`app/blog/[slug]/page.tsx` を置くと `/blog/hello` が出る。

この対応は5秒で覚えられます。私も覚えました。そのうえで、こういう質問には一切答えられませんでした。

リンクで移動したとき `layout.tsx` の状態が保たれるのはなぜか。`(marketing)` フォルダがURLから消えるのはなぜか。並列ルートで `default.tsx` を置かないとエラーになるのはなぜか。

答えられない理由は、はっきりしています。規約を**フォルダとURLの対応表**として暗記していたからです。

実際のNext.jsがやっているのは、対応づけではありません。ビルド時にフォルダ構造を1つのルートツリーへ変換し、そこから作った照合表でリクエストを解決し、クライアント遷移ではツリーの差分だけを更新する。

段階が3つあって、個々の規約はそのどこかへの入力にすぎませんでした。

Next.js 16.2のApp Routerを対象に、最小のアプリでこの3段階を追います。掲載するビルド生成物とHTTPレスポンスは、そのアプリで実際に得たものです。
...
```mermaid
flowchart LR
    A["フォルダとファイル<br/>page / layout / [slug] / (group) / @slot"] --> B["ビルド時<br/>走査してルート表とツリーを作る"]
...
レイアウトの状態が保たれるのはナビゲーション時の差分更新の話。`(marketing)` が消えるのはビルド時のツリー構築の話。`default.tsx` が要るのも、やはりナビゲーション時の話でした。

**バラバラの規約に見えていたものが、全部この3段階のどこかに収まります。**

だから新しい規約に出会ったときも、対応表に1行足すのはやめてください。代わりに、これはどの段階に効く指定なのかを置き直す。そうすると、フォルダ構造を見ただけで挙動が読めるようになります。

手元で確かめたいときに開く場所も3つだけです。`next build` の出力、`.next/routes-manifest.json`、そして `RSC: 1` を付けたリクエスト。この記事に書いたことは、全部そこに載っています。
