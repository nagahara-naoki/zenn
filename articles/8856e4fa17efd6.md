---
title: "JavaScriptのPromiseを図で理解する：値・エラー・処理順の流れ"
emoji: "⏳"
type: "tech"
...
この`return`忘れは、非同期処理そのものよりも、Promiseが次のPromiseへ何を渡すか分からないときに起こります。

Promiseは、非同期処理そのものではありません。**後で届く1回分の結果**を受け渡すための、ただのオブジェクトです。

この区別がつくと、`return`忘れ、`catch`によるエラーの握りつぶし、不要な逐次実行をコードから見つけられます。注文画面のデータ取得を題材に、値とエラーがどう流れるかを追います。
