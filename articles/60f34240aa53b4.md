---
title: "TypeScriptのanyとunknownを理解する"
emoji: "🕳️"
type: "tech"
topics: ["typescript", "any", "unknown", "型安全"]
published: true
---
...
反対の端が `never` です。どの型にも代入できますが、値がひとつも存在しません。

`any` はこの上下関係に参加していません。点線で示したように、どの型との間も両方向に通り抜けます。この出口の自由が、次の問題になります。
...
5. ジェネリクスの制約で「関数である」と言いたいだけなら `any` でよい。
6. 型が分からないから `any` にしたくなったら、分かる部分だけ書いて残りを `unknown` にできないか考える。
