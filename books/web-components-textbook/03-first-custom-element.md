...
この時点では、登録後に`name`属性を変えても表示は更新されません。属性変更を監視する方法は第6章で扱います。
...
`name`に外部入力が含まれると、このコードは文字列をHTMLとして解釈します。最初の例で`textContent`を使ったのは、文字列を文字列のまま扱うためです。HTMLを組み立てる方法とセキュリティは第16章で改めて整理します。
...
Custom ElementはHTML要素である。この一点が、以降の属性、イベント、フォーム、アクセシビリティの土台になります。
...
属性が省略された場合は「こんにちは」を使います。解けたら、次章で`define()`とアップグレードの仕組みを詳しく見ていきます。
