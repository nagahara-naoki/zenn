...
境界があるからといって、見た目を完全に固定する必要はありません。部品作者は既定の見た目を保ちながら、利用者が変更してよい範囲だけを明示できます。重要なのは「外からCSSが届くか」ではなく、「どの接点を長期的なスタイルAPIとして維持するか」です。
...
`<task-item>`内部で`<task-checkbox>`を使う場合、外側から内側のpartを直接たどることはできません。必要なpartだけを`exportparts`で転送します。
