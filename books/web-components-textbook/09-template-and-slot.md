...
`<template>`とは、その場では表示されないHTMLのひな形を書いておくための要素です。`<slot>`とは、Shadow Treeの中に置く、利用者が書いた内容の差し込み口です。
...
たとえばカードの枠、チェックボックス、余白は内部テンプレートに置けます。一方、タスク名に`strong`やリンクを含めてよいなら、その内容はSlotで受け取ります。Slotは内容を内部へコピーしません。利用者が所有するノードを、表示上の位置へ割り当てているだけです。
...
この章では、内部構造のひな形を作る`<template>`と、利用者の内容を受け取る`<slot>`を学びました。

- `<template>`の中身は画面に表示されず、`content`プロパティに`DocumentFragment`として保持されます。
- 各インスタンスでは`cloneNode(true)`で複製してからShadow Rootへ追加します。
- 配布する部品では、モジュール内で`template`を作ると利用者の準備が要りません。
- 名前のない`<slot>`は`slot`属性を持たない子を、名前付き`<slot>`は`slot="名前"`を持つ子を受け取ります。
- `<slot>`の子に書いた内容は、割り当てがない場合のフォールバックになります。
- 割り当ての変化は`slotchange`で検知し、割り当てられた要素は`assignedElements()`で取得します。
- 構造化された表示内容はSlot、プログラムが扱うデータはプロパティという分担にします。
