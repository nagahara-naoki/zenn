...
この章では、Custom Elementが内部にどう状態を持ち、その状態から表示をどう更新するかを扱います。ここでいう状態とは、要素が表示のために覚えている値のことです。レンダリングは、その状態を内部DOMへ反映する処理を指します。

Custom Elementsの仕様は、レンダリング方法を規定しません。`createElement()`で組み立てても、`innerHTML`を使っても、Litのテンプレートを使っても構いません。自由だからこそ、更新のたびに何を残すかを自分で決める必要があります。

本書のネイティブ実装では、内部DOMを最初に一度だけ作り、状態が変わったら変わった部分のプロパティだけを更新します。作り直しを避けると、フォーカスや入力途中の値といった、利用者が積み上げた状態を壊さずに済みます。
...
内部要素はホストと同じ寿命を持つので、接続のたびに付け直す必要はありません。`window`や`document`へ登録するリスナーは要素より長く残り得るため、『ライフサイクルコールバック』の章で見たように接続と切断で管理します。
...
この章では、Custom Elementの状態の持ち方と描画の更新方法を学びました。

- Custom Elementsはレンダリング方法を規定しないため、更新の方針は自分で決めます。
- 内部DOMはコンストラクターで一度だけ作り、要素の参照を保持しておきます。
- 状態が変わったら、その状態から計算した値を既存要素のプロパティへ書き込みます。
- `queueMicrotask()`を使うと、同じ同期処理内の複数の変更を1回の描画へまとめられます。
- `innerHTML`による全置換は、フォーカスや入力途中の値を捨て、XSSの入口にもなります。
- 内部要素へのリスナーはコンストラクターで登録し、要素の外へ登録するものだけ接続と切断で管理します。
- 保存する状態は、ほかの値から復元できないものに絞ります。

次章では、ここで作った内部DOMの境界そのもの、つまりShadow DOMの規則を詳しく読み解きます。
