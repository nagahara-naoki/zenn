...
[『テンプレートの記法とDirective概論』の章](./06-template-and-directive-intro)で学んだデータバインディングを思い出してください。`{{ name() }}`と書けば、`name`の値が画面に表示されます。では、`name`の値が後から変わったとき、その新しい値を画面に反映するのは誰でしょうか。ブラウザは、JavaScriptの変数が変わったことを自動では知りません。誰かが「`name`が変わったから、対応する部分のDOMを書き換えよう」と動く必要があります。
...
[『データフローとinput()・output()』の章](./08-data-flow-io)で見たように、アプリケーションはComponentの階層、すなわちツリー構造をなしています。変更検知は、そのツリーの頂点（ルート）から始まり、親から子へと順に降りていきます。各Componentで、テンプレートに書かれたバインディング（`{{ }}`や`[prop]`）の値を、前回の値と比べます。値が変わっていれば、対応するDOMを更新します。変わっていなければ、何もしません。
...
変更検知の方式は、Angularの歴史とともに変わってきました。[『Angularとは何か』の章](./02-angular-intro)で触れた流れを、変更検知の視点から振り返ると、モダンAngularの位置づけが見えてきます。
...
この「参照で比べる」という点は、次節のOnPushを理解する鍵です。配列の中身を書き換えても、配列そのものの参照が同じなら「変わっていない」と判断されることがあります。新しい配列へ差し替えると参照が変わり、変更として検知されます。
...
- **テンプレート内の`async`パイプが新しい値を受け取ったとき**: [『Directiveの実装とPipe』の章](./07-directive-and-pipe)で学んだ`async`パイプが値を流した場合
...
### Default Change DetectionとOnPushでよくあるつまずき
...
### Zone.jsとNgZone時代のAngularでよくあるつまずき
