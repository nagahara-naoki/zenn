...
:::message
TanStack Tableは、2026年8月に次のメジャーバージョンであるv9の安定版が公開されました。状態管理の仕組みがTanStack Storeに置き換わり、機能ごとにコードを取り込む形（tree-shakable）になっています。大きなテーブルでのメモリ使用量が改善され、再描画の制御も細かくなりました。
...
TanStack Tableは表の計算を担い、HTMLと見た目の決定はアプリ側に残します。
