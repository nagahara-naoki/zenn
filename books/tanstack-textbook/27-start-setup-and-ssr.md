...
TanStack QueryやTanStack Tableは、必要な場所へ追加するライブラリでした。Queryを入れても、Reactアプリの起動方法やビルド方法は変わりません。

Startは、アプリ全体を動かすフレームワークです。開発サーバーを起動し、ルートを読み取り、ブラウザ用とサーバー用のコードを別々にビルドします。SSRを使うルートへのリクエストを受けたら、どのルートを表示するか決め、必要なデータを取得してHTMLを返します。ブラウザへ移ったあとの画面遷移も、同じルート定義で処理します。

ひとことで表すと、**TanStack Startは、Reactアプリをブラウザとサーバーの両方で動かすためのフルスタックフレームワーク**です。

ここでいうフルスタックとは、画面とAPIを全部自動で作ってくれるという意味ではありません。画面の表示、HTMLの生成、データベースへのアクセス、認証情報の確認といった処理を、1つのReactプロジェクトへ置けるという意味です。

ReactとTanStack Routerだけでも、ブラウザで動くSPAは作れます。Startを加えると、同じルートとコンポーネントを使いながら、最初のHTMLをサーバーで作ったり、ブラウザからサーバーの処理を呼んだりできるようになります。

### Startはアプリの実行基盤を受け持つ

| やりたいこと | Startが用意する仕組み |
|---|---|
| ページのHTMLをサーバーで作る | SSRとストリーミング |
| データベースや秘密情報を扱う | Server Functions |
| 外部サービス向けのHTTP APIを作る | Server Routes |
| 認証やログ記録を共通化する | Middleware |
| ブラウザ用とサーバー用のコードを作る | クライアント・サーバーそれぞれのビルド |

土台にあるのはTanStack Routerです。URLとルート、Loader、Search ParamsといったRouterの仕組みはそのまま使います。Startは、そのRouterへサーバーで実行する仕組みを足します。本書ではViteを使い、ブラウザ用とサーバー用のコードをビルドします。

Startがデータベースや認証サービスまで提供するわけではありません。どのデータベースを使うか、どのようにログインさせるかはアプリ側で決めます。Startが担当するのは、Reactアプリをサーバーでも動かし、ブラウザとサーバーをつなぐところです。

### コードは3つの場所で動く

Startのコードを読むときは、「このコードはどこで動くか」を考えます。実行場所は、大きく3つに分かれます。

| 実行場所 | 例 |
|---|---|
| ブラウザ | クリック処理、DOM操作、`useState`による表示状態 |
| ブラウザとサーバーの両方 | ルートコンポーネント、共通の型や表示ロジック |
| サーバー | Server Functionの`handler`、データベース接続、秘密の環境変数 |

SSRを使うルートのコンポーネントは、最初のアクセスではサーバーでHTMLを作るために実行されます。そのHTMLがブラウザへ届くと、同じコンポーネントがブラウザでも動き、クリックなどの操作を引き継ぎます。この引き継ぎがハイドレーションです。

一方、Server Functionの`handler`はサーバーだけで動きます。ブラウザから関数を呼んだように見えても、Startが通信へ変換し、処理本体はサーバーで実行します。この境界があるため、データベースの接続情報や秘密鍵をブラウザへ渡さずに済みます。

### StartはほかのTanStackを置き換えない

Startを導入しても、サーバーから取得したデータのキャッシュはQuery、URLの状態はRouter、入力値はFormが担当します。Startは、それらが動く範囲をサーバーまで広げます。

たとえば、RouterのLoaderが画面を表示する前にデータ取得を始め、Queryが取得結果をキャッシュし、Startがその結果を含むHTMLをサーバーで作ります。3つは同じ仕事を奪い合うのではなく、取得開始、キャッシュ、HTML生成を分担します。

### Startが向くアプリ

検索結果に載せたいページ、最初の表示を早くしたいページ、Cookieを読んで表示を変えるページでは、サーバーでHTMLを作れるStartが役立ちます。データベースや外部APIを呼ぶ処理を、画面と同じTypeScriptプロジェクトへ置きたい場合にも向いています。

社内ツールのように検索エンジンを意識せず、すでに独立したAPIサーバーがあり、画面をすべてブラウザで描画しても困らないなら、TanStack RouterによるSPAだけで十分な場合もあります。Startは必須ではありません。SSRやサーバー処理が必要になったときに選ぶ道具です。
...
## Routerの知識をStartでも使える

Startは、ルーティングの土台としてTanStack Routerを使います。そこへ、サーバー側の機能が加わります。
