...
本章では、`GET`、`POST`、`PUT`、`PATCH`、`DELETE`を中心に、イベント予約APIの操作へ適切なメソッドを割り当てます。
...
- [ ] `GET`で業務状態を変更していない
- [ ] 作成先をサーバーが決める場合に`POST`を使っている
- [ ] `PUT`の省略フィールドの扱いが明確である
- [ ] `PATCH`のメディアタイプと変更規則を決めている
- [ ] `DELETE`後の利用者から見た状態を定義している
- [ ] 安全性と冪等性をAPIドキュメントへ記載している
- [ ] 非冪等な`POST`の再送対策を考えている
- [ ] 業務操作をリソースにする理由を説明できる

## メソッドが持つ意味を利用する

HTTPメソッドを正しく選ぶと、クライアントは操作の性質を推測でき、再試行やキャッシュを安全に判断できます。名前の好みではなく、安全性、冪等性、置換と部分変更の違いから選びます。

次章では、処理結果を伝えるステータスコードを整理します。成功と失敗を大分類し、クライアントが次の行動を決められるレスポンスを作ります。
