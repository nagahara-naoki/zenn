...
本章では、リソースの粒度を決め、関連データを埋め込むか、IDやリンクで参照するかを判断します。
...
```json
{
  ...
  "venue": {"id":"venue_789","name":"Harbor Hall","...":"..."},
...
完全な住所、設備、アクセス方法が必要なら、`GET /venues/venue_789`を取得します。
...
API設計と実装は分けて考えますが、提供すると約束した表現を現実的なコストで作れるかは確認します。バッチ取得、結合、読み取り専用モデル、キャッシュなどを使い、一覧件数が増えても予測可能な処理量にします。
...
- [ ] 一画面のために過剰な往復が発生しない
- [ ] 件数が増え続ける関連を無条件に埋め込んでいない
- [ ] 権限の異なるデータを分離している
- [ ] 埋め込み要約と詳細で型の意味が変わらない
- [ ] 履歴上の値か現在値かを決めている
- [ ] `include`の対象と深さを制限している
- [ ] 循環する埋め込みを作っていない
- [ ] サーバー内部でN+1が起きない実装を見込める

## 一緒に使う情報だけを近づける

適切な粒度は、テーブルの分割数では決まりません。利用頻度、サイズ、更新頻度、権限、ライフサイクルを見て、一緒に返す情報と別に取得する情報を選びます。

次章では、選んだ表現の細部をそろえます。名前、ID、日時、金額、列挙値、`null`を統一し、異なる言語のクライアントでも誤解しにくいデータを作ります。
