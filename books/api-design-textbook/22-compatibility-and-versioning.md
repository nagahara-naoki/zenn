...
ここからは、公開後のAPIを壊さずに変え、他システムと接続し、運用する方法を扱います。
...
旧版と新版を一つの実装へ条件分岐で詰め込むと、修正の影響を予測できません。境界で版ごとの入力・出力モデルを持ち、内部の共通業務処理へ変換します。
...
バージョン番号を付けるだけでは、変更は安全になりません。並行提供、移行状況の観測、旧版の停止条件まで決める必要があります。
