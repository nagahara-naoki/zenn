...
良いAPIでは、利用者が次の操作を予測できます。機能の多さだけで、使いやすさは決まりません。
...
一度覚えた規則を別の操作にも応用できれば、利用者は業務処理に集中できます。予測を支えるのは、名前だけでなく、データの形、成功時の応答、エラー、一覧操作まで含めた一貫性です。
...
答えは一つに決まりません。ただし、選んだ表現をスタイルガイドへ残せば、次のエンドポイントでも同じ判断を使えます。予測可能なAPIとは、一度覚えた規則を再利用できるAPIです。
