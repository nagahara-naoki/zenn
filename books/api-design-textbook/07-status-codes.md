...
何でも`200 OK`で返してJSONの`success`を見る設計では、HTTPクライアント、監視、プロキシが結果を正しく判断できません。本章では、イベント予約APIで頻出するコードを、利用者の次の行動と結びつけます。
...
flowchart LR
    ...
    P["問題タイプ<br/>event-full"] --> A["アプリが次の行動を選ぶ"]
```
...
- [ ] 大分類が責任範囲と合っている
- [ ] 作成完了と非同期受付を`201`と`202`で分けている
- [ ] `204`にレスポンスボディを付けていない
- [ ] 認証失敗と権限不足を分けている
- [ ] 業務競合と入力不備を分けて考えた
- [ ] 条件付きリクエストの失敗に`412`を使っている
- [ ] レート制限や一時停止に再試行情報を付けている
- [ ] サーバー障害をクライアントエラーへ変換していない

## コードから次の行動を選べるようにする

ステータスコードは、結果を短く分類する共通言語です。クライアントが修正、再認証、競合解消、待機、障害報告のどれを選ぶべきか判断できるコードを返します。

次章からは、HTTPの道具を使って業務を表現します。まず、要件から拾ったイベントや予約を、どの粒度のリソースとして公開するかを決めます。
