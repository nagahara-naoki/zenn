...
ここからは、APIの意図をHTTPで正確に伝える方法を学びます。

Web APIはHTTPの上に作られます。HTTPを単なる通信路として扱うと、メソッド、ステータスコード、キャッシュ、条件付きリクエストといった既存の仕組みを活かせません。

HTTP/1.1、HTTP/2、HTTP/3ではデータの運び方が異なります。一方、メソッドやステータスコードが表す意味は共通です。まず、HTTPメッセージがどの情報を運ぶかを押さえます。
...
APIでは通常JSONだけを提供します。それでも、リソースそのものとJSON表現は区別して考えます。すると、レスポンスに含める情報、言語や権限による表現の違い、ETagが識別する対象を整理しやすくなります。
...
HTTPの各要素には役割があります。独自ルールを足す前に、メソッド、URI、ヘッダー、ステータスコードで意図を表せないか確認すると、汎用クライアントや中継者も契約を理解できます。
