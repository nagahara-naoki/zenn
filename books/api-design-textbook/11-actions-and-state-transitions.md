...
業務操作には、実行できる条件、状態遷移、権限、後続処理、履歴があります。本章では、CRUDへ無理に押し込まず、その意味を失わないAPIを設計します。
...
- [ ] 許される状態遷移を図または表にした
- [ ] 単純な属性更新と業務操作を分けた
- [ ] 操作の記録を後から参照する必要があるか確認した
- [ ] 遷移できない場合のエラーを決めた
- [ ] 同期的に確定する処理と後続の副作用を分けた
- [ ] 長時間処理をジョブとして追跡できる
- [ ] サーバーが状態遷移と認可を最終検証する

## 業務上の出来事を失わずに表す

CRUDはAPI設計の便利な出発点ですが、業務のすべてではありません。公開、取消、中止の条件と履歴が重要なら、状態遷移や独立リソースとして表します。

次章からは、決めたリソースと操作へ具体的な入出力を与えます。まず、パス、クエリ、ヘッダー、ボディへ何を置き、入力値をどこまで受け付けるかを設計します。
