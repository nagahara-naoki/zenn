...
イベント予約APIの仕様に、次の1行しか書かれていなかったとします。

```text
POST /reservations
```

この1行から分かるのは、「予約を作る操作らしい」ということだけです。このままではクライアントを実装できません。少なくとも、次の情報が必要です。

- イベントと席数をどのJSONで送るか
- 予約が完了したら、どのステータスコードとデータが返るか
- 満席や受付終了をどう見分けるか
- 応答が届かなかったとき、安全に再送できるか

API設計とは、この不足している情報を決める作業です。何を送ればよいか、サーバーで何が起きるか、結果をどう判断するかを、クライアントが実装できるところまで具体化します。

こうして公開され、クライアントが前提とするルールのまとまりは「APIの契約」とも呼ばれます。本書でいう契約は法律上の契約書ではなく、公開後も守るAPIの仕様を指します。
...
クライアントが依存するのは、中央の公開APIです。使い方と結果を変えなければ、提供者はデータベースを移行したり、処理を複数のサービスへ分割したりできます。
...
型が`integer`であることは、使い方の一部にすぎません。値の意味、予約が成立する条件、失敗時の結果まで決まって、利用者は安全に呼び出せます。
...
## 最初の演習：1行のAPIに必要な情報を足す

冒頭の`POST /reservations`へ戻ります。次の問いに答えられれば、クライアントが迷わず使えるAPIに近づきます。
