...
APIの設計をURLから始めると、必要な操作の抜けや、役割の曖昧なエンドポイントが生まれます。先に整理するのは、利用者、目的、業務上の制約、失敗です。

本章では、イベント予約サービスの要件から、最初のAPI一覧を組み立てます。この段階ではJSONの細部まで決めません。APIが引き受ける責任と境界を見つけることに集中します。
...
利用者を先に分けると、認証と認可を後付けしにくくなります。「誰が」「どの対象へ」「何をできるか」を、要件の一部として持てるからです。
...
境界を考えるときは、ひとつの操作で守るべき整合性も確認します。予約では、残席確認と席の確保を一体として扱わなければ、定員を超える可能性があります。クライアントに二つのAPIを順番に呼ばせる設計では、この整合性を守りにくくなります。
...
- [ ] 一体として守るべき整合性を確認した
- [ ] 名詞、操作、出来事を分けた
- [ ] 規模、信頼性、監査などの非機能要件を加えた
- [ ] 判断材料が足りない項目を保留として記録した

## APIの輪郭は利用者の目的から引く

要件からAPIを作るときは、画面やテーブルより先に、利用者の目的を捉えます。目的、成功、失敗、権限、非機能要件を整理すると、必要なリソースと操作が見えてきます。

次章からHTTPの仕組みに入ります。まず、リクエストとレスポンスがどの情報を運び、HTTPのバージョンが違っても何が共通しているのかを確認します。
