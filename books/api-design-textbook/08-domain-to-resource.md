...
データベースには、保存と検索に都合のよい構造があります。APIには、利用者が目的を達成しやすい構造が必要です。両者が似ることはあっても、同じ設計図から機械的に作るものではありません。

本章では、イベント予約という業務を言葉で整理し、公開するリソースと内部に隠す構造を分けます。
...
Userを公開リソースにするかは、APIの範囲によります。本書では認証基盤が利用者を管理し、イベント予約APIは`subjectId`で主体を参照する設計とします。プロフィールAPIまで同じ境界へ入れる必要はありません。
...
- [ ] 内部モデルと公開モデルを分けた
- [ ] 読み取りと書き込みのスキーマを分けた
- [ ] 外部サービス固有の概念を必要以上に漏らしていない

## 業務の言葉で公開境界を作る

リソースは、データの置き場所ではなく、利用者が理解し操作する業務概念です。内部の保存構造を一度業務モデルへ戻し、APIとして安定させたい境界を選びます。

次章では、選んだリソースをどこまで大きく返すかを考えます。埋め込みとリンク、一覧と詳細、関連の取得を使い分け、過剰な通信と細かすぎる通信の間を調整します。
