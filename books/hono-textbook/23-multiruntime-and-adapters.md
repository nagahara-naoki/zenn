...
大切なのは、Honoアプリケーションの中に何でも直接書かないことです。ランタイム固有の処理を外側に寄せるほど、アプリケーション本体は読みやすく、テストしやすくなります。
...
実際のアプリケーションでは、リクエストごとに毎回設定を作るより、Serviceを作る段階で注入するほうが扱いやすいこともあります。大切なのは、環境変数の読み方をアプリケーション全体へ散らさないことです。
