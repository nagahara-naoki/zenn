...
ここで大切なのは、`new Hono()`しただけでは、まだAPIの機能は何もないということです。`app.get()`や`app.post()`でルートを登録して、初めてリクエストを処理できるようになります。
...
実際の内容は、テンプレートのバージョンによって少し違うことがあります。大切なのは、`dev`スクリプトでWorkersに近いローカル開発環境を起動できることです。
