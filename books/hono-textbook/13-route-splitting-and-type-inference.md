...
ただし、ここまでのコードにはひとつ問題があります。
それは、すべてを `src/index.ts` に書き続けると、すぐに見通しが悪くなることです。
...
この章では、Honoアプリケーションを複数のファイルに分けます。

ただし、単にファイルを分けるだけではありません。
Honoでは、ルート定義から型を推論できます。
この型推論を壊さずに分割することが大切です。
