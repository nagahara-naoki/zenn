...
Honoは、TypeScriptやJavaScriptでWeb APIやWebアプリケーションを作るためのフレームワークです。

最初にひとことで言うなら、Honoは「Web標準APIを土台にした、軽量で高速なWebフレームワーク」です。Cloudflare Workers、Deno、Bun、Node.jsなど、複数のJavaScriptランタイムで動かせることも大きな特徴です。
...
この章では、まだ細かいコードには入りません。まず、Honoがどのような考え方で作られているのか、どんな開発に向いているのか、ExpressのようなNode.js中心のフレームワークと何が違うのかを整理します。
...
ここで大切なのは、「完全に同じコードが、どんな環境でも何も考えずに動く」という意味ではないことです。データベース、環境変数、ファイルシステム、暗号化API、リクエストの入口などは、ランタイムごとに違いが出ます。
