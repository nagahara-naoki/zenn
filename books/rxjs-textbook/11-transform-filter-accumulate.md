...
なぜ避けるかというと、役割を混ぜると、コードの意図が読みにくくなるからです。`map`は「入力から出力への変換」に徹し、ログのような副作用は、次に見る`tap`に任せます。役割をきれいに分けておくと、あとから読んだとき、何をしている行なのかが一目瞭然になります。
...
前の状態`state`を直接書き換えず、`{ count: ... }`という新しいオブジェクトを毎回返しています。だから出力は`{ count: 1 }`、`{ count: 3 }`と、更新のたびに別のオブジェクトになります。こうすると、いつどの状態が流れたかが明確になり、あとから追いやすくなります。この「操作を受けて状態を更新する」蓄積の形は、実は状態管理の考え方そのものです。Subjectによる状態管理の章で、再び登場します。

## まとめ

この章では、値を変換・選別・蓄積するOperatorを確認しました。
