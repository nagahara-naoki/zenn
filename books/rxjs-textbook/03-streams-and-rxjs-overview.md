...
前章では、時間とともに複数の値が届く処理を、これまでのJavaScriptは素直に扱えなかった、と確認しました。
...
`error`と`complete`は、どちらもストリームの「終わり」を伝えます。ここが大事なのですが、この2つは起こるのは一度きりで、そのあとには`next`が流れません。正常に終われば`complete`、途中で異常が起きれば`error`です。
...
`|`が`complete`、`X`が`error`を表します。この2つが同時に起こることはありません。ストリームは、必ずこのどちらか一方で終わるか、あるいは終わらずに流れ続けるか、のどれかです。
...
いちばんの主役は、Observableです。Observableは「値をこう流す」という設計図であり、それ自体は何もしません。ここが初学者のつまずきどころなので、強調しておきます。Observableは、設計図を書いただけの状態です。Observerを渡して購読（subscribe）して、はじめて値が流れ始めます。この「購読するまで動かない」という性質は、この先の章で何度も確認します。
...
この章では、ストリームの考え方とRxJSの全体像を確認しました。
...
- 主役はObservableで、購読するまで動きません。処理は「作る→変換する→購読する→受け取る→終わる」の順に組み立てます。
