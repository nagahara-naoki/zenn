...
RxJSでは、不要になった処理を購読解除で上流へ伝えます。ただし、購読を解除すれば元の処理まで必ず止まるわけではありません。タイマーの停止や通信の中断は、そのObservableに適切な後片付けが実装されている場合に限られます。詳しくは「Subscription・購読解除・Observableの自作」と「エラー処理・再試行・キャンセル」の章で扱います。
...
| 非同期（あとで手に入る） | `Promise` | AsyncIterable・Observable |

この表は、非同期処理を理解するうえでとても役に立ちます。関数の戻り値は「すぐ手に入る、1つの値」。配列は「すぐ手に入る、複数の値」。`Promise`は「あとで手に入る、1つの値」です。クリックの連続、WebSocketの通知、一定間隔のタイマーは、右下の「あとで手に入る、複数の値」に入ります。

```text
クリック: ---c--c-----c--c--------c-->
```

JavaScript標準にも、非同期に複数の値を1つずつ取り出すAsyncIterableがあります。AsyncIterableは受け取る側が次の値を要求するPull型です。一方、イベントのように生み出す側のタイミングで値が届くPush型を、変換・合成・購読解除まで含めて扱うのがRxJSのObservableです。
...
RxJSでは、この「流れ」をObservableで表し、「待つ」「検索する」といった変換をOperatorで表します。先ほどの命令型のコードは、RxJSでは1本のストリームとして書けます。入力を待つ部分は「件数と時間を制御する」、通信へ切り替える部分は「Flattening Operator」の章で完成させます。
...
- クリックやWebSocketのような「時間とともに複数の値が届く」処理は、1つの値で完了する`Promise`だけでは表現できません。
