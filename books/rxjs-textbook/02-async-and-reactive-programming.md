...
RxJSでは、不要になった処理を購読解除で上流へ伝えます。ただし、購読を解除すれば元の処理まで必ず止まるわけではありません。タイマーの停止や通信の中断は、そのObservableに適切な後片付けが実装されている場合に限られます。詳しくは「購読解除とObservableの自作」と「エラー・再試行・処理の終了」の章で扱います。
...
RxJSでは、この「流れ」をObservableで表し、「待つ」「検索する」といった変換をOperatorで表します。先ほどの命令型のコードは、RxJSでは1本のストリームとして書けます。入力を待つ部分は「件数と時間の制御」、通信へ切り替える部分は「非同期処理の4つの実行方法」の章で完成させます。
